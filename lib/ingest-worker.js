...
parentPort.on('message', data => {
  console.log(`${(new Date()).toISOString()}\tworker merging ${data.measures.length} measures with ${data.spatial.length} locations`)
  const results = []
  ...
  for (const measure of data.measures) {
    ...
    if (results.length % 1000 === 0) console.log(`${(new Date()).toISOString()}\t${results.length}/${data.measures.length}`)
  }
  console.log(`${(new Date()).toISOString()}\tworker returning ${results.length} measurements`)
