...
import { getFlowSummary, getFlowTemporalRange, getFlowData } from './datastore.js'
import { ingestFlow, ingestAirBeam, progress } from './ingest.js'
...
router.get('/', async (req, res) => {
  try {
    const temporal = await getFlowTemporalRange()
    if (!temporal) return res.redirect('./import')
    const summary = await getFlowSummary(temporal.min, temporal.max)
