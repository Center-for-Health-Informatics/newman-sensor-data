...
function measurementClass (m, s) {
  if (m.no2 > s.no2.avg || m.voc > s.voc.avg || m.pm10 > s.pm10.avg || m.pm25 > s.pm25.avg || m.pm1 > s.pm1.avg) {
    return 'high'
  } else if (m.no2 < s.no2.avg && m.voc < s.voc.avg && m.pm10 < s.pm10.avg && m.pm25 < s.pm25.avg && m.pm1 < s.pm1.avg) {
    return 'low'
  } else {
    return 'medium'
  }
...
function formatGeoJSON (data, summary) {
  return {
    ...
    features: data.map(d => ({
      ...
      properties: {
        ...
        pm25: d.pm25,
        pm1: d.pm1,
        class: measurementClass(d, summary)
      }
    }))
  }
}
  ...
  try {
    ...
    res.render('summary', {
      ...
      picker: {
        ...
        months: months.map(([value, name]) => ({ name, value, selected: (value - 1) === randomDate.getMonth() })),
  ...
  try {
    ...
    if (['first', 'last', 'format'].every(key => key in req.query)) {
      ...
      switch (req.query.format) {
        ...
        case 'geojson':
          res.json(formatGeoJSON(data, getSummary(temporal.min, temporal.max)))
          break

        default:
          throw Error(`unknown export format “${req.query.format}”`)
      }
    } else if (['month', 'format'].every(key => key in req.query)) {
      const [year, month] = req.query.month.split('-', 2).map(s => Number.parseInt(s))
      ...
      switch (req.query.format) {
        case 'csv':
          res.set({
            'Content-Type': 'text/csv',
            'Content-Disposition': `attachment; filename="${req.query.month}.csv"`
          }).send(formatCSV(data))
          break
