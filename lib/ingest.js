  ...
  while (lines.length > 0) {
    ...
    if (sensorID && fields.length === columns.size) {
      measurements.push({
        ...
        latitude: Number.parseFloat(fields[columns.get('geo:long')]), // yes, this is to undo the label swap in the original data
        longitude: Number.parseFloat(fields[columns.get('geo:lat')]), // yes, this is to undo the label swap in the original data
